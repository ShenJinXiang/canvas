        ...
        FireWord: function(x, y, vx, vy, ax, ay) {
            ...
            this.draw = function(ctx) {
                if (this.status == 1) {
                    ...
                    // ctx.lineTo(this.x + this.r, this.y);
                    let t = this.path[0];
                    let linearGrad = ctx.createLinearGradient(this.x, this.y, t.x, t.y);
                    linearGrad.addColorStop(0, 'rgba(' + this.color + '0.8)');
                    linearGrad.addColorStop(1, 'rgba(' + this.color + '0)');
                    ctx.fillStyle = linearGrad;
                    ctx.moveTo(this.x - this.r, this.y);
                    ctx.lineTo(this.x + this.r, this.y);
                    ctx.lineTo(t.x + this.r, t.y);
                    ctx.lineTo(t.x - this.r, t.y);
                    ctx.closePath();
                    ctx.fill();
                    ctx.restore();
